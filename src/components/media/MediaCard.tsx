...
export default function MediaCard({ media, onAddToWatchlist, index = 0 }: MediaCardProps) {
  ...
  const itemType = media.type || (media as any).mediaType;
  
  const isAdded = isInWatchlist(media.id);
  
  const itemStatus = (media as any).status;
  const progress = (media as any).progress;
  const totalEpisodes = media.totalEpisodes || (media as any).totalEpisodes;
  ...
  return (
    ...
    >
      ...
      >
        <span className="material-symbols-outlined text-[18px]" style={isAdded ? { fontVariationSettings: "'FILL' 1" } : {}}>
          {isAdded ? 'check' : 'add'}
        </span>
      </button>
    </motion.article>
