...
export default function RecommendationsClient({
  ...
  const fallbackRecommended = [...trendingMovies.slice(0, 5), ...trendingSeries.slice(0, 5), ...trendingAnime.slice(0, 5)]
  ...
  const displayItems = recommended.length > 0 
      ...
      <header className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop w-full flex flex-col items-center text-center gap-6">
        <div className="flex flex-col gap-2 items-center">
          <h1 className="font-display-xl-mobile text-[40px] md:font-display-xl md:text-[64px] font-bold text-on-surface tracking-tight">
            Your <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">Recommendations</span>
          </h1>
        ...
        {loading ? (
          <div className="flex flex-col gap-4 mx-margin-mobile md:mx-margin-desktop overflow-hidden mt-8">
            ...
            <div className="flex gap-4">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div key={i} className="min-w-[140px] md:min-w-[200px] aspect-[2/3] bg-surface-variant/30 rounded-xl shimmer border border-white/5" />
              ))}
            </div>
          </div>
        ) : error ? (
